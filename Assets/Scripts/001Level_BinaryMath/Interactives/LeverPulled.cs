...
public class LeverPulled : MonoBehaviour
{
    ...
    private void Update()
    {
        if (isActive)
        {
            ...
            if (zeroPosition.localPosition.z > onePosition.localPosition.z)
            {
                ...
                else transform.localPosition = new Vector3(startPosition.x, leverTopYLocalPosition.localPosition.y, transform.localPosition.z);
            }
            else
            {
                if (transform.localPosition.z < zeroPosition.localPosition.z) SetLeverToZeroPosition();
            ...
            //rebuilt lever system off center rotation of 0 so its easy to find angle hence use cosine to find where to fix y position of levelTop
            transform.localRotation = Quaternion.identity;

            //update lever shaft rotation based on user grabbing top of lever
            localRotationX = RescaleValues(leverTopMin, leverTopMax, leverBaseMin, leverBaseMax, transform.localPosition.z);
            ...
            localRotationX = RescaleValues(leverTopMin, leverTopMax, numberWheelMin, numberWheelMax, transform.localPosition.z);
    ...
    public void Deactivate()
    {
        ...
        leverGlow.material.SetColor("_EmissionColor", Color.red);
        foreach (GameObject element in unlockedElements)
        {
            element.SetActive(false);
        }
    ...
    private float RescaleValues(float OldMin, float OldMax, float NewMin, float NewMax, float OldValue)
    {
